...
<%
PropVO propVO = (PropVO) session.getAttribute("propVO");
%>

<%
PropOptService propOptSvc = new PropOptService();
List<PropOptVO> list = propOptSvc.getAll();

List<PropOptVO> filteredList = list.stream().filter(opt -> opt != null && opt.getProposalId() == propVO.getProposalId())
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/front_end/assets/css/common.css">
...
<body>
			...
			<div class="container">
				...
				<body>
					<div class="form-wrapper">
						<h1 class="mb-4">選擇欲上架商品選項</h1>
						<form METHOD="post" ACTION="<%=request.getContextPath()%>/front_end/prop/selectProposalOption.do" name="form1"
							enctype="multipart/form-data">

							<div class="input-group input-group-sm mb-3">
								<div class="input-group-prepend">
									<span class="input-group-text" id="inputGroup-sizing-sm">提案名稱</span>
									<%=propVO.getProposalName()%>
									
									<input type= "hidden" name ="proposalId" value="<%=propVO.getProposalId()%>">
								</div>
							...
							<table class="table table-hover">
								...
								<thead>
									<tr>
										...
										<th scope="col">商品選項編號</th>
										<th scope="col">商品選項名稱</th>
										<th scope="col">商品價格</th>
										<th scope="col">勾選欲上架選項</th>

									</tr>
								</thead>
								<tbody>
									<c:forEach var="propOptVO" items="${filteredList}">
										<tr>
											<th scope="row">${propOptVO.proposalOptionId}</th>
											...
											<div class="form-check form-check-inline">
													...
													<input
														type="checkbox" class="form-check-input" name="options" value="${propOptVO.proposalOptionId}">勾選
													</label></td>
	...
	<script src="<%=request.getContextPath()%>/front_end/assets/js/picture.js"></script>
